...
async function loadCategories() {
  ...
  itemCategory.innerHTML = cats.map(c => `<option value="${c.id}">${c.name}</option>`).join('');
}
...
itemForm.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      alert('خطا: ' + (errData.error || 'مشکل در ذخیره‌سازی'));
      return;
    }
    alert('ذخیره شد ✅');
...
function resetLibraryCache() {
  libraryCache = { files: [], total: 0, offset: 0, isLoading: false, hasMore: true };
...
async function loadMediaLibrary(reset = true) {
  ...
  try {
    const res = await fetch(`${apiBase}/media?limit=20&offset=${libraryCache.offset}`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
...
function setupLibraryObserver() {
  ...
  libraryObserver = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      if (entry.isIntersecting && libraryCache.hasMore && !libraryCache.isLoading) {
        loadMediaLibrary(false);
      }
    });
  }, { root: mediaModal.querySelector('.media-library-body'), rootMargin: '0px', threshold: 0.1 });
